...
//ATTENTION Il faut modifier ici l'heure actuelle et votre position eventuelle.
//La position actuelle est par rapport à la tour Eiffel.
...
//Début calibration vers le Soleil
Wire.beginTransmission(0x0D); //ici se trouve l'addresse du magnétomètre peut varier entre composants
compass.read();
compass.getAzimuth(); //Ligne nécessaire car la première valeur est toujours un zéro
compass.read();
while(compass.getAzimuth()!=int(az)){
if(compass.getAzimuth()>int(az))MotorH.Step(false);
else if(compass.getAzimuth()<int(az)) MotorH.Step(true);
compass.read();
delay(40); 
}
 Wire.endTransmission();
//Fin calibration vers le Soleil
Degrees(MotorV,ha);
